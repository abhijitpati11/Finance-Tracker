...
export default function FinanceContextProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const getSavingsData = async () => {
      ...
      const savingsData = docSnap.docs.map((doc) => {
        return {
          id: doc.id,
          ...doc.data(),
        };
  ...
  const addMonthlyDatabase = async (savingsDetails) => {
    ...
    // adding data to the collectionref
    try {
      const res = await addDoc(collectionRef, savingsDetails);
  ...
  // delete savings data 
  const removeSavingsData = async (id) => {
